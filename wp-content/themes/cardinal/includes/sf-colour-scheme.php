<?php
	...
	*/
	...
	function sf_get_current_color_scheme_id() {
		...
		$sf_cardinal_options = get_option ( 'sf_cardinal_options' );
		
		if ( isset( $sf_cardinal_options['colour_scheme_select_scheme'] ) ) {
			return $sf_cardinal_options['colour_scheme_select_scheme'];
		} else {
			return 0;
		}
	...
	function sf_get_schema_info($schema_id) {
		...
		if ( $schema_id ) {
			...
			$schemas = get_option ( 'sf_cardinal_options_schemes_saved' );
...
//			$dropdown_values = array('' => '') + $dropdown_values; 
	...
	function sf_add_schema($schema_array = array()) {
		...
		$schemas = get_option ( 'sf_cardinal_options_schemes_saved' ); // get list of schemas
		...
		update_option ( 'sf_cardinal_options_schemes_saved', $schemas );
...
//			print_r ($schemas); exit;
	...
	function sf_get_schema_select_html() {
		...
		$schemas_array = get_option ( 'sf_cardinal_options_schemes_saved' );
		
		$html = '<select name="sf_cardinal_options[colour_scheme_select_scheme]" id="colour_scheme_select_scheme"> ';
...
//		echo "++++".$schema_id;
		...
		$html = "<ul id=\"sf_cardinal_options-color-list\" class=\"color-list\">";
	...
	function sf_download_file($content = null, $file_name = null) {
	    ...
	    if (! wp_verify_nonce($_REQUEST['nonce'], 'sf_cardinal_options_schema_action') ) 
	...
	function sf_create_export_download_link($echo = false){
	    $site_url = home_url();
	    $args = array(
	        'sf_cardinal_options_schema_action' => 'export_schema',
	        'nonce' => wp_create_nonce('sf_cardinal_options_schema_action')
	    );
	...
	function sf_add_query_var_vars() {
	    global $wp;
	    $wp->add_query_var('sf_cardinal_options_schema_action');
	}
	...
	function sf_admin_redirect_download_files(){
	    ...
	    if (array_key_exists('sf_cardinal_options_schema_action', $wp->query_vars) && $wp->query_vars['sf_cardinal_options_schema_action'] == 'export_schema') {
	        sf_download_file();
	        die();
	    } else if (array_key_exists('sf_cardinal_options_schema_action', $wp->query_vars) && $wp->query_vars['sf_cardinal_options_schema_action'] == 'import_scheme') {
	    	// db_import_scheme();
	    	die();
	    }
	}
