	...
	*/
	...
	if (!function_exists('sf_site_loading')) {
		function sf_site_loading() {
			...
			$siteloading = '<div id="site-loading" class="'.$page_transition.'">';
			$siteloading .= '<div class="spinner">';
			if ($page_transition == "wave") {
			$siteloading .= '<div class="rect1"></div>';
			$siteloading .= '<div class="rect2"></div>';
			$siteloading .= '<div class="rect3"></div>';
			$siteloading .= '<div class="rect4"></div>';
			$siteloading .= '<div class="rect5"></div>';
			} else if ($page_transition == "circle") {
			$siteloading .= '<div class="spinner-container container1">';
			$siteloading .= '<div class="circle1"></div>';
			$siteloading .= '<div class="circle2"></div>';
			$siteloading .= '<div class="circle3"></div>';
			$siteloading .= '<div class="circle4"></div>';
			$siteloading .= '</div>';
			$siteloading .= '<div class="spinner-container container2">';
			$siteloading .= '<div class="circle1"></div>';
			$siteloading .= '<div class="circle2"></div>';
			$siteloading .= '<div class="circle3"></div>';
			$siteloading .= '<div class="circle4"></div>';
			$siteloading .= '</div>';
			$siteloading .= '<div class="spinner-container container3">';
			$siteloading .= '<div class="circle1"></div>';
			$siteloading .= '<div class="circle2"></div>';
			$siteloading .= '<div class="circle3"></div>';
			$siteloading .= '<div class="circle4"></div>';
			$siteloading .= '</div>';
			} else if ($page_transition == "three-bounce") {
			$siteloading .= '<div class="bounce1"></div>';
			$siteloading .= '<div class="bounce2"></div>';
			$siteloading .= '<div class="bounce3"></div>';
			}
			$siteloading .= '</div>';
			$siteloading .= '</div>';		
			echo $siteloading;
		}
		add_action('sf_before_page_container', 'sf_site_loading', 0);
	}
	...
	if (!function_exists('sf_header')) {
		function sf_header($header_layout) {
			...
			$header_output = $header_left_output = $header_right_output = $main_menu = '';
			...
			$header_left_output .= '<li class="menu-search parent"><a href="#" class="header-search-link-alt"><i class="ss-search"></i></a>'. "\n";
			$header_left_output .= '<div class="ajax-search-wrap" data-ajaxurl="'.$ajax_url.'"><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/">';
			if ($header_search_pt != "any") {
			$header_left_output .= '<input type="hidden" name="post_type" value="'.$header_search_pt.'" />';
			}
			$header_left_output .= '<input type="text" placeholder="'.__("Search", "swiftframework").'" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>'. "\n";
			$header_left_output .= '</li>'. "\n";
			...
			$header_right_output .= '<li class="menu-search parent"><a href="#" class="header-search-link-alt"><i class="ss-search"></i></a>'. "\n";
			$header_right_output .= '<div class="ajax-search-wrap" data-ajaxurl="'.$ajax_url.'"><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/">';
			if ($header_search_pt != "any") {
			$header_right_output .= '<input type="hidden" name="post_type" value="'.$header_search_pt.'" />';
			}
			$header_right_output .= '<input type="text" placeholder="'.__("Search", "swiftframework").'" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>'. "\n";
			...
			$header_output .= '<div class="container">'. "\n";
			$header_output .= '<div class="row">'. "\n";
			$header_output .= sf_logo('col-sm-8 logo-left');
			$header_output .= '<div class="header-right col-sm-4">'.$header_right_output.'</div>'. "\n";			
			$header_output .= '</div> <!-- CLOSE .row -->'. "\n";
			$header_output .= '</div> <!-- CLOSE .container -->'. "\n";
			$header_output .= '</header>'. "\n";
	...
	if (!function_exists('sf_main_menu')) {
		function sf_main_menu($id, $layout = "") {
			...
			if ($layout == "full") {	
				...
				if ($header_search_type == "search-on") {
				$menu_full_output .= '<li class="menu-search parent"><a href="#" class="header-search-link-alt"><i class="ss-search"></i></a>'. "\n";
				$menu_full_output .= '<div class="ajax-search-wrap" data-ajaxurl="'.$ajax_url.'"><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/">';
				if ($header_search_pt != "any") {
				$menu_full_output .= '<input type="hidden" name="post_type" value="'.$header_search_pt.'" />';
				}
				$menu_full_output .= '<input type="text" placeholder="'.__("Search", "swiftframework").'" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>'. "\n";
				$menu_full_output .= '</li>'. "\n";			
				}
				if ($show_cart) {
				...
				$menu_with_search_output .= '<div class="ajax-search-wrap" data-ajaxurl="'.$ajax_url.'"><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/">';
				if ($header_search_pt != "any") {
				$menu_with_search_output .= '<input type="hidden" name="post_type" value="'.$header_search_pt.'" />';
				}
				$menu_with_search_output .= '<input type="text" placeholder="'.__("Search", "swiftframework").'" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>'. "\n";
				...
				if ($layout == "float-2") {
					...
					if ($header_search_type == "search-on") {
					$menu_float_output .= '<li class="menu-search parent"><a href="#" class="header-search-link-alt"><i class="ss-search"></i></a>'. "\n";
					$menu_float_output .= '<div class="ajax-search-wrap" data-ajaxurl="'.$ajax_url.'"><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/">';
					if ($header_search_pt != "any") {
					$menu_float_output .= '<input type="hidden" name="post_type" value="'.$header_search_pt.'" />';
					}
					$menu_float_output .= '<input type="text" placeholder="'.__("Search", "swiftframework").'" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>'. "\n";
					$menu_float_output .= '</li>'. "\n";			
					}
					if ($show_cart) {
				...
				if ($header_search_type == "search-on") {
				$menu_vert_output .= '<li class="menu-search parent"><a href="#" class="header-search-link-alt"><i class="ss-search"></i></a>'. "\n";
				$menu_vert_output .= '<div class="ajax-search-wrap" data-ajaxurl="'.$ajax_url.'"><div class="ajax-search-results"></div><div class="ajax-loading"></div><form method="get" class="ajax-search-form" action="'.home_url().'/">';
				if ($header_search_pt != "any") {
				$menu_vert_output .= '<input type="hidden" name="post_type" value="'.$header_search_pt.'" />';
				}
				$menu_vert_output .= '<input type="text" placeholder="'.__("Search", "swiftframework").'" name="s" autocomplete="off" /></form></div>'. "\n";
				$menu_vert_output .= '</li>'. "\n";			
				}
				if ($show_cart) {
	...
	if (!function_exists('sf_get_cart')) {
		function sf_get_cart() {
			...
			if (sf_woocommerce_activated()) {
				...
				$cart_output .= '<li class="parent shopping-bag-item"><a class="cart-contents" href="'.$woocommerce->cart->get_cart_url().'" title="'.__("View your shopping cart", "swiftframework").'"><i class="ss-cart"></i>'.$cart_total.'<span class="num-items">'.$cart_count_text.'</span></a>';
