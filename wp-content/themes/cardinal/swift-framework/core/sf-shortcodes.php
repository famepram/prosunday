<?php
	...
	*/
	...
	function sf_icon($atts, $content = null) {
		...
		if (substr( $image, 0, 3 ) === "ss-" || substr( $image, 0, 3 ) === "fa-") {
			$icon = $image;
			$image = "";
		}
		...
		if ($image != "") {
			...
			if ($cont == "yes") {
				if ($character != "") {
					$icon_output .= '<div class="sf-icon-cont cont-'.$size.' sf-icon-float-'.$float.'"><span class="sf-icon-character sf-icon sf-icon-'.$size.'" style="color:'.$color.';">'.$character.'</span></div>';
				} else {
					$icon_output .= '<div class="sf-icon-cont cont-'.$size.' sf-icon-float-'.$float.'"><i class="'.$icon.' sf-icon sf-icon-'.$size.'" style="color:'.$color.';"></i></div>';
				}
			} else {
				if ($character != "") {
	...
	function sf_iconbox($atts, $content = null) {
		...
		$icon_box .= do_shortcode('[icon size="large" icon="'.$icon.'" character="'.$character.'" image="'.$image.'" float="none" cont="no" color="'.$icon_color.'"]');
		}
		
		if ($type == "standard") {
		$icon_box .= do_shortcode('[icon size="large" icon="'.$icon.'" character="'.$character.'" image="'.$image.'" float="none" cont="yes" color="'.$icon_color.'" link="'.$link.'" target="'.$target.'"]');
		} else if ($type == "left-icon") {
		$icon_box .= do_shortcode('[icon size="small" icon="'.$icon.'" character="'.$character.'" image="'.$image.'" float="none" cont="yes" color="'.$icon_color.'" link="'.$link.'" target="'.$target.'"]');
		} else if ($type == "left-icon-alt") {
		$icon_box .= do_shortcode('[icon size="medium" icon="'.$icon.'" character="'.$character.'" image="'.$image.'" float="none" cont="no" color="'.$icon_color.'" link="'.$link.'" target="'.$target.'"]');
		} else if ($type == "boxed-one" || $type == "boxed-three") {
		$icon_box .= do_shortcode('[icon size="medium" icon="'.$icon.'" character="'.$character.'" image="'.$image.'" float="none" cont="yes" color="'.$icon_color.'" link="'.$link.'" target="'.$target.'"]');
		} else if ($type != "boxed-two" && $type != "boxed-four" && $type != "standard-title" && $type != "animated") {
		$icon_box .= do_shortcode('[icon size="large" icon="'.$icon.'" character="'.$character.'" image="'.$image.'" float="none" cont="no" color="'.$icon_color.'" link="'.$link.'" target="'.$target.'"]');
		}
	...
	function sf_chart($atts) {
		...
		if ($size == "70") { $linewidth = "3"; }
		if ($size == "170") { $linewidth = "3"; }
	...
	function sf_fullscreen_video($atts, $content = null) {
		...
		if ($type == "image-button" || $type == "image-button2" || $type == "image-button3") {
			...
			if ($type == "image-button3") {
				$fw_video_output .= '<i class="ss-video"></i>';			
			} else {
	...
	function sf_social_share($atts) {
		...
		if ($center == "yes") {
			$share_output = '<div class="article-share share-center" data-buttontext="'.__("Share this", "swiftframework").'" data-image="'.$image.'"></div>';
		} else {
			$share_output = '<div class="article-share" data-buttontext="'.__("Share this", "swiftframework").'" data-image="'.$image.'"></div>';	
		}
	...
	function sf_gallery($attr) {
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			if ( ! empty( $attr['link'] ) && 'file' === $attr['link'] ) {
				$image_output .= '<a href="'.$image_file_lightbox_url.'" class="lightbox" rel="ilightbox[galleryid-'.$id.']" title="'.$image_alt.'"></a>';
