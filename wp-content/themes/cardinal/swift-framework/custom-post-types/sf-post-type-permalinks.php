	...
	*/
	...
	class sf_post_type_permalinks {
		...
		public function settings_init() {
			...
			add_settings_section( 'sf-portfolio-permalink', __( 'Portfolio permalink base', 'swift-framework-admin' ), array( $this, 'portfolio_settings' ), 'permalink' );
	
			// Add portfolio settings
			add_settings_field('portfolio_category_slug', __( 'Portfolio category base', 'swift-framework-admin' ), array( $this, 'portfolio_category_slug_input' ), 'permalink', 'optional');
			
			// Add galleries section to the permalinks page
			add_settings_section( 'sf-galleries-permalink', __( 'Galleries permalink base', 'swift-framework-admin' ), array( $this, 'galleries_settings' ), 'permalink' );
	
			// Add galleries settings
			add_settings_field('galleries_category_slug', __( 'Galleries category base', 'swift-framework-admin' ), array( $this, 'galleries_category_slug_input' ), 'permalink', 'optional');
			
			// Add team section to the permalinks page
			add_settings_section( 'sf-team-permalink', __( 'Team permalink base', 'swift-framework-admin' ), array( $this, 'team_settings' ), 'permalink' );
	
			// Add team settings
			add_settings_field('team_category_slug', __( 'Team category base', 'swift-framework-admin' ), array( $this, 'team_category_slug_input' ), 'permalink', 'optional');
		}
	
		public function portfolio_category_slug_input() {
			$port_permalinks = get_option( 'sf_portfolio_permalinks' );
			?>
			<input name="portfolio_category_slug" type="text" class="regular-text code" value="<?php if ( isset( $port_permalinks['category_base'] ) ) echo esc_attr( $port_permalinks['category_base'] ); ?>" placeholder="<?php echo __('portfolio-category', 'swift-framework-admin') ?>" />
			<?php
		}
		public function galleries_category_slug_input() {
			$gallery_permalinks = get_option( 'sf_galleries_permalinks' );
			?>
			<input name="galleries_category_slug" type="text" class="regular-text code" value="<?php if ( isset( $gallery_permalinks['category_base'] ) ) echo esc_attr( $gallery_permalinks['category_base'] ); ?>" placeholder="<?php echo __('galleries-category', 'swift-framework-admin') ?>" />
			<?php
		}
		public function team_category_slug_input() {
			$team_permalinks = get_option( 'sf_team_permalinks' );
			?>
			<input name="team_category_slug" type="text" class="regular-text code" value="<?php if ( isset( $team_permalinks['category_base'] ) ) echo esc_attr( $team_permalinks['category_base'] ); ?>" placeholder="<?php echo __('team-category', 'swift-framework-admin') ?>" />
			<?php
		}
		...
		public function portfolio_settings() {
			...
			<table class="form-table">
				<tbody>
					<tr>
						<th><label><input name="portfolio_permalink" type="radio" value="<?php echo $structures[0]; ?>" class="sf_port_tog" <?php checked( $structures[0], $portfolio_permalink ); ?> /> <?php _e( 'Default', 'swift-framework-admin' ); ?></label></th>
						<td><code><?php echo home_url(); ?>/?portfolio=sample-portfolio-item</code></td>
					</tr>
					<tr>
						<th><label><input name="portfolio_permalink" type="radio" value="<?php echo $structures[1]; ?>" class="sf_port_tog" <?php checked( $structures[1], $portfolio_permalink ); ?> /> <?php _e( 'Portfolio', 'swift-framework-admin' ); ?></label></th>
						<td><code><?php echo home_url(); ?>/<?php echo $portfolio_base; ?>/sample-portfolio-item/</code></td>
					</tr>
					<?php if ( $portfolio_page ) : ?>
						<tr>
							<th><label><input name="portfolio_permalink" type="radio" value="<?php echo $structures[2]; ?>" class="sf_port_tog" <?php checked( $structures[2], $portfolio_permalink ); ?> /> <?php _e( 'Portfolio base', 'swift-framework-admin' ); ?></label></th>
			...
			<script type="text/javascript">
				jQuery(function(){
					jQuery('input.sf_port_tog').change(function() {
						jQuery('#sf_portfolio_permalink_structure').val( jQuery(this).val() );
					});
	
					jQuery('#sf_portfolio_permalink_structure').focus(function(){
		...
		public function galleries_settings() {
			echo wpautop( __( 'These settings control the permalinks used for galleries. These settings only apply when <strong>not using "default" permalinks above</strong>.', 'swift-framework-admin' ) );
			...
			$base_slug 		= __( 'galleries', 'swift-framework-admin' );
			$galleries_base 	= __( 'galleries', 'swift-framework-admin' );
			...
			<table class="form-table">
				<tbody>
					<tr>
						<th><label><input name="galleries_permalink" type="radio" value="<?php echo $structures[0]; ?>" class="sf_gal_tog" <?php checked( $structures[0], $galleries_permalink ); ?> /> <?php _e( 'Default', 'swift-framework-admin' ); ?></label></th>
						<td><code><?php echo home_url(); ?>/?galleries=sample-gallery</code></td>
					</tr>
					<tr>
						<th><label><input name="galleries_permalink" type="radio" value="<?php echo $structures[1]; ?>" class="sf_gal_tog" <?php checked( $structures[1], $galleries_permalink ); ?> /> <?php _e( 'Galleries', 'swift-framework-admin' ); ?></label></th>
						<td><code><?php echo home_url(); ?>/<?php echo $galleries_base; ?>/sample-gallery/</code></td>
					</tr>
					<tr>
						<th><label><input name="galleries_permalink" id="sf_galleries_custom_selection" type="radio" value="custom" class="sf_gal_tog" <?php checked( in_array( $galleries_permalink, $structures ), false ); ?> />
							<?php _e( 'Custom Base', 'swift-framework-admin' ); ?></label></th>
						<td>
							<input name="galleries_permalink_structure" id="sf_galleries_permalink_structure" type="text" value="<?php echo esc_attr( $galleries_permalink ); ?>" class="regular-text code"> <span class="description"><?php _e( 'Enter a custom base to use. A base <strong>must</strong> be set or WordPress will use default instead.', 'swift-framework-admin' ); ?></span>
						</td>
					</tr>
				</tbody>
			</table>
			<script type="text/javascript">
				jQuery(function(){
					jQuery('input.sf_gal_tog').change(function() {
						jQuery('#sf_galleries_permalink_structure').val( jQuery(this).val() );
					});
	
					jQuery('#sf_galleries_permalink_structure').focus(function(){
		...
		public function team_settings() {
			...
			<table class="form-table">
				<tbody>
					<tr>
						<th><label><input name="team_permalink" type="radio" value="<?php echo $structures[0]; ?>" class="sf_team_tog" <?php checked( $structures[0], $team_permalink ); ?> /> <?php _e( 'Default', 'swift-framework-admin' ); ?></label></th>
						<td><code><?php echo home_url(); ?>/?team=sample-team-member</code></td>
					</tr>
					<tr>
						<th><label><input name="team_permalink" type="radio" value="<?php echo $structures[1]; ?>" class="sf_team_tog" <?php checked( $structures[1], $team_permalink ); ?> /> <?php _e( 'Team', 'swift-framework-admin' ); ?></label></th>
						<td><code><?php echo home_url(); ?>/<?php echo $team_base; ?>/sample-team-member/</code></td>
					</tr>
					<tr>
						<th><label><input name="team_permalink" id="sf_team_custom_selection" type="radio" value="custom" class="sf_team_tog" <?php checked( in_array( $team_permalink, $structures ), false ); ?> />
							<?php _e( 'Custom Base', 'swift-framework-admin' ); ?></label></th>
						<td>
							<input name="team_permalink_structure" id="sf_team_permalink_structure" type="text" value="<?php echo esc_attr( $team_permalink ); ?>" class="regular-text code"> <span class="description"><?php _e( 'Enter a custom base to use. A base <strong>must</strong> be set or WordPress will use default instead.', 'swift-framework-admin' ); ?></span>
						</td>
					</tr>
				</tbody>
			</table>
			<script type="text/javascript">
				jQuery(function(){
					jQuery('input.sf_team_tog').change(function() {
						jQuery('#sf_team_permalink_structure').val( jQuery(this).val() );
					});
	
					jQuery('#sf_team_permalink_structure').focus(function(){
