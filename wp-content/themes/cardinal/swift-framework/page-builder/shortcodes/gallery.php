...
class SwiftPageBuilderShortcode_spb_gallery extends SwiftPageBuilderShortcode {
    ...
    protected function content($atts, $content = null) {
		   	...
		   	$title = $width = $el_class = $gallery_id = $output = $items = $masonry_gallery = $main_slider = $thumb_slider = $el_position = $gallery_images = $thumb_images = '';
	        ...
	        while ( $gallery_query->have_posts() ) : $gallery_query->the_post();
   			    ...
   			    if ($display_type == "masonry") {
			       	...
			       	foreach ( $gallery_images as $image ) {
			       		...
			       		$masonry_gallery .= '<figure class="animated-overlay">';
			       					       		
			       		if ($enable_lightbox == "yes") {
			       	    $masonry_gallery .= '<a href="'.$image['full_url'].'" class="lightbox" rel="ilightbox[gallery-'.$gallery_id.']"></a>';
			       	    }
			       	...
			       	foreach ( $gallery_images as $image ) {
			       		...
			       		if ($enable_lightbox == "yes") {
			       	    $main_slider .= "<li><a href='{$image['full_url']}' class='lightbox' rel='ilightbox[galleryid-".$gallery_id."]'><img src='{$image['url']}' width='{$image['width']}' height='{$image['height']}' alt='{$image['alt']}' /></a>";
