	...
	*/
	...
	class SwiftPageBuilderShortcode_spb_image extends SwiftPageBuilderShortcode {
	    ...
	    public function content( $atts, $content = null ) {
	        ...
	        $el_class = $width = $image_size = $animation = $frame = $lightbox = $image_link = $link_target = $caption = $fullwidth = $el_position = $el_class = $image = '';
			...
			$output .= '<figure class="animated-overlay caption-hover clearfix" '.$image_width.'>';
	        ...
	        if ($image_link != "") {
		        ...
		        $output .= $img['thumbnail'];
		        $output .= '<a class="lightbox" href="'.$img_url[0].'" rel="ilightbox['.$image.']"></a>';
		        $output .= '<div class="figcaption-wrap"></div>';
		        $output .= '<figcaption>';
		        if ($caption_pos == "hover") {
		        	$output .= '<div class="thumb-info">';
	...
	SPBMap::map( 'spb_image', array(
	    ...
	    "params"	=> array(
			...
			array(
			    ...
			    "value" => array(__("Hover", "swift-framework-admin") => "hover", __("Below", "swift-framework-admin") => "below"),
	...
	class SwiftPageBuilderShortcode_spb_gmaps extends SwiftPageBuilderShortcode {
	    ...
	    public function contentAdmin($atts, $content = null) {
	        ...
	        if ( count($tab_titles) ) {
	            ...
	            foreach ( $tab_titles as $tab ) {
	                $tmp .= '<li><a href="#tab-'. sanitize_title( $tab[0] ) .'"><span>' . $tab[0] . '</span></a></li>';
	            }
	            $tmp .= '</ul>';
	        } else {
	   ...
	   public function content( $atts, $content = null ) {
	        ...
	        $output .= '<div class="spb_map_wrapper"><div class="map-canvas" style="width:100%;height:'.$size.'px;"  data-zoom="'.$zoom.'" data-maptype="'.$type.'" data-mapcolor="'.$color.'" data-mapsaturation="'.$saturation.'"></div>';
	...
	SPBMap::map( 'spb_gmaps', array(
		...
		"params"	=> array(
	        ...
	        array(
	            ...
	            "value" => array(__("Color", "swift-framework-admin") => "color", __("Mono (Light)", "swift-framework-admin") => "mono-light", __("Mono (Dark)", "swift-framework-admin") => "mono-dark"),
	    ...
	    'default_content' => '
		<ul>
			<li><a href="#tab-1"><span>'.__('Pin 1', 'swift-framework-admin').'</span></a></li>
		
		</ul>
	
		<div id="tab-1" class="row-fluid spb_column_container spb_sortable not-column-inherit not-sortable">
		
				[spb_map_pin width="1/1"] '.__('This is a map pin. Click the edit button to change it.', 'swift-framework-admin').' [/spb_map_pin]
		</div>',
	...
	SPBMap::map( 'spb_directory', array(
		...
		"params"	=> array(
			...
			array(
	            ...
	            "value" => array(__("Above", "swift-framework-admin") => "above", __("Below", "swift-framework-admin") => "below"),
			...
			array(
	            ...
	            "value" => array(__("Map", "swift-framework-admin") => "map", __("List", "swift-framework-admin") => "list" , __("Map & List", "swift-framework-admin") => "maplist"),
