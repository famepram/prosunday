<?php
	...
	*/
			...
			<div class="campaign-detail row">
				...
				<div class="col-sm-4">
					...
					<div class="campaign-meta">
						<time class="date" datetime="<?php echo $post_date_str; ?>"><span><?php _e('Launched', 'swiftframework'); ?></span><?php echo $post_date; ?></time>
						<?php if ( ! $campaign->is_endless() ) { ?>
						<div class="funding-ends"><span><?php _e('Funding Ends', 'swiftframework'); ?></span><?php echo $end_date ?></div>
						<?php } ?>
					<?php if ( $campaign->location() ) : ?>
						<div class="location"><span><?php _e('Location', 'swiftframework'); ?></span><?php echo $campaign->location(); ?></div>
					<?php endif; ?>
					</div>
				</div>
			</div>
	...
	if (!function_exists('sf_campaign_sidebar')) {
		...
		?>
				<div id="back-this-project" class="campaign-sidebar col-sm-4">
					<?php 
	...
	if (!function_exists('sf_campaign_info')) {
		...
		<div class="author-info-wrap clearfix">
			...
			<div class="author-bio">
				<div class="author-name" itemprop="author" itemscope itemtype="http://schema.org/Person"><h3><a itemprop="name" href="<?php echo get_author_posts_url( $author->ID, $author->user_nicename ); ?>"><?php the_author_meta('display_name'); ?></a></h3></div>
				<div class="author-bio-text">
					<?php the_author_meta('description'); ?>
				</div>
				<?php if ($campaign->contact_email() != "") { ?>
				<a href="mailto:<?php echo $campaign->contact_email(); ?>" class="sf-button contact-author accent"><?php _e( 'Contact Author', 'swiftframework' ); ?></a>
				<?php } ?>
			</div>
		</div>
		...
		<div class="campaign-info post-info clearfix">
			<?php if ($categories) { ?>
			<div class="categories-wrap"><?php _e("Categories:", "swiftframework"); ?><span class="categories"><?php echo $categories; ?></span></div>	
			<?php } ?>
			<?php if (has_tag()) { ?>
			...
			<?php } ?>
				...
				?>
					<div class="next-article">
						...
						<div class="blog-item-details"><?php echo sprintf(__('<span class="author">By <a href="%2$s" rel="author" itemprop="author">%1$s</a></span> in %3$s on <time datetime="%4$s">%5$s</time>', 'swiftframework'), $post_author, get_author_posts_url(get_the_author_meta( 'ID' )), $post_categories, $post_date_str, $post_date); ?></div>
					</div>
				<?php } ?>
	...
	if (!function_exists('sf_campaign_backers')) {
		function sf_campaign_backers() {
			...
			<div class="backers-wrap container">
				<div id="campaign-backers" class="<?php echo $backers_class; ?>">
					...
					<?php } else { ?>
						...
						<ol class="backer-list">
							<?php foreach ( $backers as $backer ) : ?>
								<?php
									$payment_id = sf_get_post_meta( $backer->ID, '_edd_log_payment_id', true );
