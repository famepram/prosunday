<?php
	...
	*/
	...
	if (!function_exists('sf_portfolio_items')) { 
		function sf_portfolio_items($display_type, $multi_size_ratio, $fullwidth, $gutters, $columns, $show_title, $show_subtitle, $show_excerpt, $hover_show_excerpt, $excerpt_length, $item_count, $category, $exclude_categories, $pagination, $sidebars) {
	...
	if (!function_exists('sf_portfolio_thumbnail')) { 
		function sf_portfolio_thumbnail($display_type = "gallery", $multi_size = "", $multi_size_ratio = "1/1", $columns = "2", $hover_show_excerpt = "no", $excerpt_length = 20, $gutters = "yes", $fullwidth = "no") {
			...
			$portfolio_thumb = $thumb_image = $thumb_gallery = $video = $item_class = $link_config = $port_hover_style = '';
			$thumb_width = 400;
			$thumb_height = 300;
			$video_height = 300;
			...
			if ($display_type == "gallery" || $display_type == "masonry-gallery" || $display_type == "multi-size-masonry") {
			$portfolio_thumb .= '<figure class="animated-overlay">'. "\n";
			} else {
				...
				if ($image) {
					...
					if ($display_type == "gallery" || $display_type == "masonry-gallery" || $display_type == "multi-size-masonry") {
						if ($hover_show_excerpt == "yes") {
							$portfolio_thumb .= '<h4 itemprop="name headline">'.$item_title.'</h4>';
							if ($post_excerpt != "") {
								$portfolio_thumb .= '<div class="name-divide"></div>';
								$portfolio_thumb .= '<div itemprop="description">'.$post_excerpt.'</div>';
							}
						} else {
							$portfolio_thumb .= '<h4 itemprop="name headline">'.$item_title.'</h4>';
							if ($item_subtitle != "") {
								$portfolio_thumb .= '<div class="name-divide"></div>';
								$portfolio_thumb .= '<h5 itemprop="name alternativeHeadline">'.$item_subtitle.'</h5>';
							}
						}
					} else {
	...
	if (!function_exists('sf_portfolio_item_link')) { 
		function sf_portfolio_item_link() {
			...
			if ($thumb_link_type == "link_to_url") {
				...
				$link_config = 'href="'.$thumb_img_url.'" class="lightbox" rel="ilightbox[portfolio]"';
				...
				$link_config = 'href="'.$lightbox_image_url.'" class="lightbox" rel="ilightbox[portfolio]"';			
				$item_icon = apply_filters('sf_port_lightbox_icon', "ss-view");
