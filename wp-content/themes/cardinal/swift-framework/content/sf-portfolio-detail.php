	...
	*/
	...
	if (!function_exists('sf_portfolio_detail_media')) { 
		function sf_portfolio_detail_media() {
			...
			<?php } else { ?>
			<figure class="media-wrap container">
			<?php } ?>
	...
	if (!function_exists('sf_portfolio_item_details')) {  
		function sf_portfolio_item_details() {
			...
			<?php } ?>
				...
				<?php } ?>
				<?php if ($client != "") { ?>
				<div class="client"><span><?php _e("Client:", "swiftframework"); ?></span><?php echo $client; ?></div>
				<?php } ?>
				<time class="date updated" itemprop="datePublished" datetime="<?php echo get_the_date('Y-m-d');?>"><span><?php _e("Date:", "swiftframework"); ?></span><?php echo get_the_date();?></time>
				<?php if ($item_link != "") { ?>
				...
				<?php } ?>
				<div class="article-share" data-buttontext="<?php _e("Share this", "swiftframework"); ?>" data-image="<?php echo $image; ?>"></div>
			</section>
	...
	if (!function_exists('sf_portfolio_related_projects')) {  
		function sf_portfolio_related_projects() {
			...
			?>
			<section class="related-projects <?php echo $wrap_class; ?> clearfix">
				
				<h2 class="<?php echo $heading_class; ?>"><?php _e("Related Projects", "swiftframework"); ?></h2>
				...
				<?php while ( $related->have_posts() ): $related->the_post(); ?>
			        ...
			        <article class="<?php echo $item_class; ?>">
			        	<figure class="animated-overlay">
			        		...
			        		<figcaption <?php echo $port_hover_style; ?>>
			        			<div class="thumb-info">						
			        				<h4><?php echo $item_title; ?></h4>
			        				<div class="name-divide"></div>
			        				<h5><?php echo $item_subtitle; ?></h5>
			        			</div>
			        		</figcaption>
			        	</figure>
			        </article>
