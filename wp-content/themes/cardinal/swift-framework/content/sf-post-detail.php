	...
	*/
	...
	if (!function_exists('sf_post_detail_media')) {
		function sf_post_detail_media() {
			...
			if ($fw_media_display == "fw-media-title" && $media_type != "none") {
				...
				?>
				<div class="detail-feature" style="background-image: url(<?php echo $image_url; ?>); min-height: <?php echo $custom_media_height; ?>px;">
					<div class="details-overlay">
						...
						<div class="post-item-details" <?php echo $details_overlay_styling; ?>><?php echo sprintf(__('In %1$s on <time datetime="%2$s" itemprop="datePublished" class="updated">%3$s</time>', 'swiftframework'), $post_categories, $post_date_str, $post_date); ?></div>
						<?php } else if (!$remove_dates) { ?>
						<div class="post-item-details" <?php echo $details_overlay_styling; ?>><?php echo sprintf(__('<span class="vcard author">By <span itemprop="author" class="fn">%1$s</span></span> in %2$s <time class="date updated" datetime="%3$s" itemprop="datePublished">%4$s</time>', 'swiftframework'), $post_author, $post_categories, $post_date_str, $post_date); ?></div>
						<?php } else if (!$single_author) { ?>
						<div class="post-item-details" <?php echo $details_overlay_styling; ?>><?php echo sprintf(__('<span class="vcard author">By <span itemprop="author" class="fn">%1$s</span></span> in %2$s', 'swiftframework'), $post_author, $post_categories); ?></div>
						<?php } ?>
					</div>
				</div>
			<?php } else if ($fw_media_display == "fw-media" && $media_type != "none")  {
				sf_get_template('detail-media');
			} else if ($fw_media_display == "standard" && $media_type != "none" || $sf_sidebar_config == "one-sidebar")  { ?>
		...
		function sf_post_download() {
			...
			if ($download_button) { ?>
				<div class="post-download">
					...
					<a href="<?php echo wp_get_attachment_url($download_file); ?>" class="sf-button accent"><?php echo $download_text; ?></a>
					<?php } ?>
				</div>
			<?php }
		}
	...
	if (!function_exists('sf_post_info')) {
		...
		<?php } ?>
			...
			<div class="categories-wrap"><?php _e("Categories:", "swiftframework"); ?><span class="categories"><?php $post_categories; ?></span></div>	
			<?php } ?>
			<?php if (has_tag()) { ?>
			...
			<div class="comments-likes">
				...
				<div class="comments-wrapper"><a href="#comments" class="smooth-scroll-link"><i class="ss-chat"></i><span><?php comments_number(__('0 Comments', 'swiftframework'), __('1 Comment', 'swiftframework'), __('% Comments', 'swiftframework')); ?></span></a></div>
				<?php } ?>
			...
			<?php } ?>
				...
				?>
					<div class="next-article">
						...
						<?php if ($single_author && !$remove_dates) { ?>
							<div class="blog-item-details"><?php echo sprintf(__('In %1$s on <time datetime="%2$s">%3$s</time>', 'swiftframework'), $post_categories, $post_date_str, $post_date); ?></div>
						<?php } else if (!$remove_dates) { ?>
							<div class="blog-item-details"><?php echo sprintf(__('<span class="author">By <a href="%2$s" rel="author" itemprop="author">%1$s</a></span> in %3$s on <time datetime="%4$s">%5$s</time>', 'swiftframework'), $author_name, $author_url, $post_categories, $post_date_str, $post_date); ?></div>
						<?php } else if (!$single_author) { ?>
							<div class="blog-item-details"><?php echo sprintf(__('<span class="author">By <a href="%2$s" rel="author" itemprop="author">%1$s</a></span> in %3$s', 'swiftframework'), $author_name, $author_url, $post_categories); ?></div>
						<?php } ?>
					</div>
				<?php } ?>
			...
			if( $related_posts_query->have_posts() ) {
				echo '<div class="related-wrap">';						
					echo '<div class="related-articles '.$related_articles_class.'">';						
						echo '<h2 class="heading">'.__("Related Articles", "swiftframework").'</h2>';
						echo '<div class="related-items recent-posts posts-type-bold no-gutters clearfix">';
						
						while ( $related_posts_query->have_posts() ) : $related_posts_query->the_post();
							echo sf_get_recent_post_item($post, 'bold', 20, 'col-sm-3');
						endwhile;
