<?php
	...
	*/
	...
	if (!function_exists('sf_galleries')) { 
		function sf_galleries($display_type, $link_type, $fullwidth, $gutters, $columns, $show_title, $show_subtitle, $show_excerpt, $excerpt_length, $item_count, $category, $pagination, $sidebars) {
			...
			while ( $galleries_items->have_posts() ) : $galleries_items->the_post();								
				...
				/* LINK TYPE CONFIG
				================================================== */
				$item_link = sf_gallery_item_link($link_type, $count);
				...
				================================================== */
				if ($thumb_type != "none") {
					$gallery_items_output .= sf_gallery_thumbnail($display_type, $link_type, $columns, $gutters, $count);
				}
	...
	if (!function_exists('sf_gallery_filter')) { 
		function sf_gallery_filter($style = "basic", $parent_category = "") {
			...
			if ($parent_category == "" || $parent_category == "All") {
				$tax_terms = sf_get_category_list('gallery-category', 1);
			} else {
				$tax_terms = sf_get_category_list('gallery-category', 1, $parent_category);
			}
	...
	if (!function_exists('sf_gallery_thumbnail')) { 
		function sf_gallery_thumbnail($display_type = "gallery", $link_type = "lightbox", $columns = "2", $gutters = "yes", $count = 0) {
			...
			if ($display_type == "gallery" || $display_type == "masonry-gallery") {
			$gallery_thumb .= '<figure class="animated-overlay">'. "\n";
			} else {
