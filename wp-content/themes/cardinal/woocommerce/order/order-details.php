...
<div class="my-account-right">
	...
	<?php if ( get_option('woocommerce_allow_customers_to_reorder') == 'yes' && $order->status=='completed' ) : ?>
		<p class="order-again">
			<a href="<?php echo esc_url( $woocommerce->nonce_url( 'order_again', add_query_arg( 'order_again', $order->id, add_query_arg( 'order', $order->id, get_permalink( woocommerce_get_page_id( 'view_order' ) ) ) ) ) ); ?>" class="button"><?php _e( 'Order Again', 'swiftframework' ); ?></a>
		</p>
	<?php endif; ?>
	...
	<?php
		if ($order->billing_email) echo '<dt>'.__( 'Email:', 'swiftframework' ).'</dt><dd>'.$order->billing_email.'</dd>';
		if ($order->billing_phone) echo '<dt>'.__( 'Telephone:', 'swiftframework' ).'</dt><dd>'.$order->billing_phone.'</dd>';
	?>
	</dl>
	...
	<?php endif; ?>
			...
			<address><p>
				<?php
					if (!$order->get_formatted_billing_address()) _e( 'N/A', 'swiftframework' ); else echo $order->get_formatted_billing_address();
				?>
			</p></address>
	...
	<?php if (get_option('woocommerce_ship_to_billing_address_only')=='no') : ?>
		...
		<div class="col-2">
			...
			<address><p>
				<?php
					if (!$order->get_formatted_shipping_address()) _e( 'N/A', 'swiftframework' ); else echo $order->get_formatted_shipping_address();
				?>
			</p></address>
