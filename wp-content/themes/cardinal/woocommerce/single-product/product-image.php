		...
		<ul class="slides">
			<?php
				if ( has_post_thumbnail() ) {
					...
					$image = sf_aq_resize( $image_link, 600, NULL, true, false);
					...
					echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<li itemprop="image">%s<a href="%s" itemprop="image" class="woocommerce-main-image zoom lightbox" rel="ilightbox[product]" title="%s" alt="%s"><i class="ss-expand"></i></a></li>', $image_html, $image_link, $image_title, $image_alt ), $post->ID );	
				...
				if ( $attachment_ids ) {
					...
					foreach ( $attachment_ids as $attachment_id ) {
						...
						if ($image) {
							...
							echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<li>%s<a href="%s" class="%s lightbox" rel="ilightbox[product]" title="%s" alt="%s"><i class="ss-expand"></i></a></li>', $image_html, $image_link, $image_class, $image_title, $image_alt ), $attachment_id, $post->ID, $image_class );
